# uploader.py atualizado com validação, feedback visual e suporte a múltiplas bases
...
# Colunas obrigatórias (exemplo; você pode ajustar com base na estrutura definitiva)
colunas_essenciais = [
...
def validar_colunas(df, nome_arquivo):
    ...
    if faltantes:
        st.warning(f"\U000026A0\ufe0f Arquivo '{nome_arquivo}' está faltando colunas essenciais: {faltantes}")
        return False
    return True
...
def show():
    ...
    if uploaded_files:
        ...
        if dfs:
            ...
            towrite.seek(0)
            st.download_button("Download do Excel Unificado", data=towrite, file_name="dados_unificados.xlsx", mime="application/vnd.ms-excel")
        else:
